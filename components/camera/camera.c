...
esp_err_t camera_probe(const camera_config_t* config, camera_model_t* out_camera_model)
{
    ...
    ESP_LOGD(TAG, "Camera PID=0x%02x VER=0x%02x MIDL=0x%02x MIDH=0x%02x",
            id->PID, id->VER, id->MIDH, id->MIDL);//Camera PID=0x76 VER=0x73 MIDL=0x7f MIDH=0xa2
...
esp_err_t camera_init(const camera_config_t* config)
{
    ...
    if (pix_format == PIXFORMAT_YUV422) {
          s_state->sensor.set_framerate(&s_state->sensor,0); //0 lowest fps first...
    ...
    if ((pix_format == PIXFORMAT_RGB565) || (pix_format == PIXFORMAT_YUV422)) {
      ...
      uint8_t highspeed_sampling_mode = 0;//change from 2 to 0  OK
      ...
      */
      ...
      if (highspeed_sampling_mode == 0) {
        ...
        s_state->sampling_mode = SM_0A0B_0C0D; // sampling mode for ov7670... works well for YUV422
    ...
    ESP_LOGD(TAG, "Frame buffer (%d bytes)", s_state->fb_size);//Frame buffer (153600 bytes)
    if (s_state->fb == NULL) {
